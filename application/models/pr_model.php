...
class PR_model extends CI_Model
{
	...
	{
           $this->db->select('completed_tasks.task_id, tasks.task, completed_tasks.date, tasks.date, tasks.colour, u1.username, u1.first_name, u1.last_name, u2.username, u2.first_name, u2.last_name');
           $this->db->from('completed_tasks');
           $this->db->join('tasks', 'completed_tasks.task_id = task.id', 'left');
           $this->db->join('users u1', 'completed_tasks.user_id = t.id', 'left');
           $this->db->join('users u2', 'tasks.user_id = u2.id', 'left');
